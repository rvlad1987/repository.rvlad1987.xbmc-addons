...
import xbmcup.app, xbmcup.net, xbmcup.log
...
class TexturesSQL(SQL):
    ...
    def _delete_texture(self, url):
        self.set( "DELETE FROM texture WHERE url='{}'".format(url) )
        
    
    # https://github.com/xbmc/xbmc/blob/master/xbmc/TextureDatabase.cpp
    def AddCachedTexture(self, details):
        try:
            self._delete_texture(details.url)
            ...
            xbmcup.log.error('fail - insert texture data into db!')
        else:
            try:
                ...
                self._delete_texture(details.url)
                xbmcup.log.error('fail - insert texture data and size into db!')
